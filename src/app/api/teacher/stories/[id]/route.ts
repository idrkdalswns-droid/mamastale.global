...
export async function PATCH(
  ...
  if (Array.isArray(body.spreads)) {
    if (body.spreads.length === 0 || body.spreads.length > 20) {
      return sb.applyCookies(
        NextResponse.json({ error: t("Errors.teacher.sceneCountInvalid") }, { status: 400 })
      );
    }
    ...
    if (!validSpreads) {
      return sb.applyCookies(
        NextResponse.json({ error: t("Errors.teacher.invalidSceneFormat") }, { status: 400 })
      );
    }
    ...
    for (const s of body.spreads as Array<{ spreadNumber: number; title?: string; text: string }>) {
      ...
      if (containsProfanity(cleanText)) {
        return sb.applyCookies(
          NextResponse.json({ error: t("Errors.profanity.detected") }, { status: 400 })
        );
      }
  ...
  if (Object.keys(updates).length === 0) {
    return sb.applyCookies(
      NextResponse.json({ error: t("Errors.story.noChanges") }, { status: 400 })
    );
  }
  ...
  if (error) {
    ...
    return sb.applyCookies(
      NextResponse.json({ error: t("Errors.story.updateFailed") }, { status: 500 })
    );
  }
...
export async function DELETE(
  ...
  if (error) {
    ...
    return sb.applyCookies(
      NextResponse.json({ error: t("Errors.story.deleteFailed") }, { status: 500 })
    );
  }

  if (!deleted) {
    return sb.applyCookies(
      NextResponse.json({ error: t("Errors.story.notFoundOrDeleted") }, { status: 404 })
    );
  }
