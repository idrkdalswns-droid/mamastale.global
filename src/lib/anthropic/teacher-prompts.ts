 ...
 *  A (0-3턴): 공감적 경청 + 브리프 수집
 *  B (4-6턴): 서사 구조 + 캐릭터 설계
 *  C (7-9턴): 세부 조율 + 최종 확인
 *  10턴: 자동 생성 (done.generateReady)
 */
export function getPhaseFromTurnCount(turnCount: number): string {
  if (turnCount < 4) return "A";
  if (turnCount < 7) return "B";
  return "C";
}

/** 현재 Phase에서 남은 턴 수 */
function getRemainingTurns(turnCount: number, phase: string): number {
  const phaseEnd: Record<string, number> = { A: 4, B: 7, C: 10 };
  return Math.max(0, (phaseEnd[phase] ?? 10) - turnCount);
}
