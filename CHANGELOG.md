...
### Added
- **마지막 페이지 표지 선택 CTA**: StoryViewer 마지막 페이지에서 표지 미선택 시 "이 동화에 어울리는 표지를 골라볼까요?" CTA 표시
- **메인 서비스 CoverPickerModal 연동**: 동화 읽기 화면에서 모달로 표지 선택 가능 (기존 전이 맵 변경 없음)
- **딸깍동화 표��� 선택 지원**: DalkkakResultClient에 CoverPickerModal 추가 (인증 사용자만)
- **서재 표지 CTA**: library/[id]에서 표지 미선택 동화에 CTA 표시

### Changed
- **CoverPicker 카피 개선**: "동화의 표지를 골라주세요" → "이 동화에 어울리는 표지를 골라볼까요?"
- **CoverPicker 반응형 개선**: 메인 미리보기 max-w-[340px] → max-w-full (소형 기기 대응)
- **썸네일 크기 증가**: 88×55 → 110×70 (터치 타겟 개선)
- **건너뛰기 버튼 가시성**: "표지 없이 진행하기" 11px → "나중에 선택하기" 13px + 밑줄
...
### Fixed
- **P0 동화 손실 버그 긴급 수정**: 30분 티켓 윈도우를 4시간으로 확대하여 긴 대화 후 저장 실패 방지
- **Pre-INSERT CAS clear**: 1 티켓 = 1 동화 보장 (기존 fire-and-forget → 선제적 CAS 클리어)
- **storySaveError 구분**: "no_tickets" vs "ticket_expired" 에러 타입 분리
- **프리미엄 모달**: 저장 실패 시 불필요한 프리미엄 업그레이드 모달 숨김
- **StoryViewer 페이지 점프**: 새 동화가 항상 처음부터 시작 (storyId 기반 키 격리)
- **"내 동화 보러가기" 버튼**: 가시성 개선 (텍스트 링크 → 버튼)
...
### Added
- 신규 동화 "엄마 귀에 울린 작은 종소리" (DIY + 쇼케이스)
...
### Added
- 딸깍 동화 질문 화면에 일러스트 이미지 통합 (Q1-Q19 × 4선택지 = 76장)
- 랜딩 페이지 리디자인 — 서비스 목적/진행방식 안내 섹션 + 6장 수채화 일러스트
- 선택지 클릭 후 미니멀 로딩 스피너 ("다음 질문을 준비하고 있어요")

### Fixed
- Q19→Q20(주관식) 전환 불가 버그 수정 — Phase 5 전환 시 q20_prompt 미저장 문제
- 모바일에서 4개 선택지가 한 화면에 안 보이던 여백 과다 문제 최적화
...
### Added
- 신규 동화 "아빠는 마법이 서툴러!" DIY + 쇼케이스 등록
- 쇼케이스 이미지 최적화 (76MB → 20MB)
...
### Added
- 신규 DIY 동화 "엄마, 여기 무거워?" 등록 (stone)
- 쇼케이스 DB INSERT 마이그레이션 041 추가
- /add-story 슬래시 커맨드 CLAUDE.md에 등록
